...
let torusGroup, particles, foregroundStars, pulsingBackgroundStars, bigForegroundStars;
...
async function initialize() {
  ...
  try {
    ...
    const particleData = createParticleSystem(scene);
    particles = particleData.particles;
    foregroundStars = particleData.foregroundStars;
    pulsingBackgroundStars = particleData.pulsingBackgroundStars;
    bigForegroundStars = particleData.bigForegroundStars;
...
function animate() {
  ...
  // Vordergrund-Sterne rotieren lassen
  if (foregroundStars) {
    foregroundStars.rotation.y += 0.003; // Langsame Rotation um Y-Achse
  }
  
  // Pulsierende Hintergrund-Sterne animieren
  if (pulsingBackgroundStars) {
    pulsingBackgroundStars.children.forEach(star => {
      const userData = star.userData;
      if (userData) {
        const pulse = Math.sin(currentTime * userData.pulseSpeed + userData.pulsePhase) * 0.5 + 0.5;
        star.material.size = userData.originalSize * (0.5 + pulse);
        star.material.opacity = 0.3 + pulse * 0.5;
      }
    });
  }
  
  // Große Vordergrund-Sterne rotieren lassen (andere Richtung)
  if (bigForegroundStars) {
    bigForegroundStars.rotation.y -= 0.001; // Langsamere Rotation, andere Richtung
  }
